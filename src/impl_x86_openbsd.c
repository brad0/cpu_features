...
static void DetectFeaturesFromOs(X86Info* info, X86Features* features) {
  (void)info;
  // Handling OpenBSD platform through parsing /var/run/dmesg.boot.
  ...
  if (fd >= 0) {
    ...
    for (bool stop = false; !stop;) {
      ...
      if (!CpuFeatures_StringView_StartsWith(line, str("cpu0: "))) continue;
      // Lines of interests are of the following form:
      // "cpu0: cpuid 1 edx=bfebfbff<FPU,VME,DE,PSE,TSC,MSR,PAE,MCE,CX8,APIC>"
